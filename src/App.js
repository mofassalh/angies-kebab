  ...
  if (isAdmin) {
    ...
    return (
      <div style={{ minHeight: "100vh", background: "#f7f7f5", fontFamily: "'Nunito',sans-serif" }}>
        <style>{`* { box-sizing: border-box; margin: 0; padding: 0; } input,textarea,select { outline: none; } .del-btn:hover { background: #ff3b30 !important; color: #fff !important; } .edit-btn:hover { background: #e8520a !important; color: #fff !important; }`}</style>
        ...
        <div style={{ maxWidth: 900, margin: "0 auto", padding: 24 }}>
          <div style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 12, marginBottom: 24 }}>
            {[{ label: "Total Items", value: Object.values(menu).flat().length, icon: "🍔" }, { label: "Categories", value: Object.keys(menu).length, icon: "📋" }, { label: "Most Items", value: Object.entries(menu).sort((a,b) => b[1].length-a[1].length)[0][0], icon: "⭐" }].map((s,i) => (
          ...
          <div key={item.id} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 10 }}>
            <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
              <div style={{ display: "flex", alignItems: "center", gap: 4, background: "#f5f5f5", borderRadius: 20, padding: "2px 6px" }}>
                <button onClick={() => removeFromCart(item.id)} style={{ width: 20, height: 20, borderRadius: "50%", background: "#e0e0e0", color: "#333", fontSize: 12, display: "flex", alignItems: "center", justifyContent: "center", border: "none", cursor: "pointer" }}>−</button>
                <span style={{ fontSize: 12, fontWeight: 800, minWidth: 14, textAlign: "center" }}>{item.qty}</span>
                <button onClick={() => addToCart(item)} style={{ width: 20, height: 20, borderRadius: "50%", background: "#e8520a", color: "#fff", fontSize: 12, display: "flex", alignItems: "center", justifyContent: "center", border: "none", cursor: "pointer" }}>+</button>
              </div>
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#f7f7f5", fontFamily: "'Nunito', sans-serif", color: "#1a1a1a" }}>
      ...
      <div style={{ background: "#fff", borderBottom: "1px solid #e8e8e8", padding: "0 16px", display: "flex", alignItems: "center", gap: 12, height: 64, position: "sticky", top: 0, zIndex: 100 }}>
        <img src="/logo.jpg" alt="Angie's Logo" style={{ height: 48, width: 48, borderRadius: "50%", objectFit: "cover", border: "2px solid #f5c842", flexShrink: 0 }} />
        ...
        {!isMobile && <button style={{ padding: "6px 14px", borderRadius: 20, border: "1px solid #e0e0e0", background: "#fff", fontSize: 13, fontWeight: 600, cursor: "pointer", fontFamily: "'Nunito',sans-serif" }}>About</button>}
      </div>
              ...
              {activeCategory === "Popular" && !search.trim() && (
                <div style={{ position: "relative", height: isMobile ? 200 : 260, overflow: "hidden", margin: "16px 16px 0 16px", borderRadius: 16 }}>
                  ...
                  <div style={{ position: "absolute", bottom: 16, left: 16, color: "#fff" }}>
                    ...
                    <div style={{ fontSize: 11, opacity: 0.85 }}>🕐 Open until 11PM · 📍 Ascot Vale, 3032</div>
                  </div>
                  ...
                  <div key={cat} className="fade-in">
                    ...
                    <div style={{ display: "grid", gridTemplateColumns: isMobile ? "1fr 1fr" : "repeat(auto-fill, minmax(220px, 1fr))", gap: 12, marginBottom: 28 }}>
                      {items.map(item => {
                        ...
                        return (
                          <div key={item.id} className="food-card" style={{ background: "#fff", borderRadius: 14, overflow: "hidden", border: "1px solid #eee" }}>
                            <div style={{ position: "relative" }}>
                              ...
                              {!inCart ? (
                                <button className="add-btn" onClick={() => addToCart(item)} style={{ position: "absolute", bottom: 6, right: 6, width: 30, height: 30, borderRadius: "50%", background: "#fff", color: "#1a1a1a", fontSize: 20, display: "flex", alignItems: "center", justifyContent: "center", boxShadow: "0 2px 8px rgba(0,0,0,0.2)" }}>+</button>
                              ) : (
                                <div style={{ position: "absolute", bottom: 6, right: 6, display: "flex", alignItems: "center", gap: 3, background: "#fff", borderRadius: 20, padding: "2px 5px", boxShadow: "0 2px 8px rgba(0,0,0,0.2)" }}>
                                  <button onClick={() => removeFromCart(item.id)} style={{ width: 22, height: 22, borderRadius: "50%", background: "#f0f0f0", color: "#333", fontSize: 13, display: "flex", alignItems: "center", justifyContent: "center", border: "none", cursor: "pointer" }}>−</button>
                                  <span style={{ fontSize: 12, fontWeight: 800, minWidth: 14, textAlign: "center" }}>{inCart.qty}</span>
                                  <button onClick={() => addToCart(item)} style={{ width: 22, height: 22, borderRadius: "50%", background: "#e8520a", color: "#fff", fontSize: 13, display: "flex", alignItems: "center", justifyContent: "center", border: "none", cursor: "pointer" }}>+</button>
                                </div>
                              )}
                            </div>
      ...
      {page === "checkout" && (
        <div className="fade-in" style={{ maxWidth: 520, margin: "0 auto", padding: "20px 16px 80px" }}>
          ...
          <h2 style={{ fontSize: 22, fontWeight: 900, marginBottom: 20 }}>Pickup Checkout</h2>
          ...
          <button className="pay-btn" onClick={handlePay} disabled={paying} style={{ width: "100%", padding: "15px", borderRadius: 12, background: paying ? "#ccc" : "#1a1a1a", color: "#fff", fontSize: 15, fontWeight: 800, fontFamily: "'Nunito',sans-serif", border: "none" }}>
      ...
      {page === "confirmation" && (
        <div className="fade-in" style={{ maxWidth: 480, margin: "40px auto", padding: "0 20px", textAlign: "center" }}>
          ...
          <button className="pay-btn" onClick={() => { setCart([]); setPage("menu"); }} style={{ width: "100%", padding: "14px", borderRadius: 12, background: "#1a1a1a", color: "#fff", fontSize: 15, fontWeight: 800, fontFamily: "'Nunito',sans-serif", border: "none", cursor: "pointer" }}>Order Again 🍔</button>
        </div>
      )}
    </div>
  );
