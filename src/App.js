...
export default function App() {
  ...
  async function fetchGallery() {
    ...
    const slideImgs = data.filter(i => i.location === "all" || i.slide === true).map(i => i.image_url);
    const galleryImgs = data.map(i => i.image_url);
    if (slideImgs.length > 0) setSlides(slideImgs);
