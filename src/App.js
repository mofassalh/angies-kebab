...
export default function App() {
  ...
  const addToCart = (item, opts = {}) => {
    const cartItem = {
      ...
      addons: opts.addons || [],
      finalPrice: item.price + (opts.addons || []).reduce((s, a) => s + (a.price || 0), 0) + (opts.toppings || []).reduce((s, t) => s + (t.price || 0), 0),
    };
  ...
  const toppingExtra = selectedToppings.reduce((s, t) => s + (t.price || 0), 0);
  const addonExtra = selectedAddons.reduce((s, a) => s + (a.price || 0), 0);
  const itemTotal = selectedItem ? (selectedItem.price + toppingExtra + addonExtra) * qty : 0;

  const addToCartFromModal = () => {
    addToCart(selectedItem, { qty, meat: selectedMeat, sauce: selectedSauce, toppings: selectedToppings, addons: selectedAddons });
  ...
  const handlePay = async () => {
    ...
    setTimeout(() => { setPaying(false); setPage("confirmation"); setShowCartMobile(false); }, 1500);
  };
          ...
          <div key={item.cartId} style={{ marginBottom: 10, paddingBottom: 10, borderBottom: "1px solid #f5f5f5" }}>
            <div style={{ display: "flex", justifyContent: "space-between" }}>
              <div style={{ flex: 1 }}>
                ...
                {item.meat && <div style={{ fontSize: 11, color: "#888" }}>{item.meat} · {item.sauce}</div>}
                {item.addons && item.addons.length > 0 && <div style={{ fontSize: 11, color: "#e8520a" }}>+{item.addons.map(a => a.name).join(", ")}</div>}
              </div>
              <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                <span style={{ fontSize: 13, fontWeight: 700, fontFamily: "'Nunito',sans-serif" }}>${(item.finalPrice * item.qty).toFixed(2)}</span>
    ...
    <div style={{ minHeight: "100vh", background: "#f7f7f5", fontFamily: "'Nunito', sans-serif", color: "#1a1a1a" }}>
      ...
      <style>{`
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .dot { cursor: pointer; border: none; }
          ...
          <div className="modal-in" style={{ background: "#fff", borderRadius: "20px 20px 0 0", width: "100%", maxWidth: 560, maxHeight: "92vh", overflowY: "auto" }} onClick={e => e.stopPropagation()}>
            ...
            <div style={{ padding: "20px 20px 100px" }}>
              <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 8 }}>
                <h2 style={{ fontSize: 20, fontWeight: 900, flex: 1 }}>{selectedItem.name}</h2>
              ...
              {selectedItem.addons && selectedItem.addons.length > 0 && (
                <div style={{ marginBottom: 20, background: "#f9f9f9", borderRadius: 14, padding: 16 }}>
                  ...
                  {selectedItem.addons.map((addon, i) => {
                    ...
                    return (
                      <button key={i} onClick={() => setSelectedAddons(prev => selected ? prev.filter(a => a.name !== addon.name) : [...prev, addon])} className="option-btn" style={{ width: "100%", display: "flex", justifyContent: "space-between", alignItems: "center", padding: "10px 0", background: "none", border: "none", borderBottom: "1px solid #eee", fontFamily: "'Nunito',sans-serif" }}>
            ...
            <div style={{ position: "sticky", bottom: 0, background: "#fff", borderTop: "1px solid #eee", padding: "14px 20px", display: "flex", gap: 12 }}>
              ...
              <button onClick={addToCartFromModal} style={{ flex: 1, padding: "14px", borderRadius: 12, background: "#1a1a1a", color: "#fff", fontSize: 15, fontWeight: 800, border: "none", cursor: "pointer", fontFamily: "'Nunito',sans-serif" }}>
      ...
      <div style={{ background: "#fff", borderBottom: "1px solid #e8e8e8", padding: "0 16px", display: "flex", alignItems: "center", gap: 12, height: 64, position: "sticky", top: 0, zIndex: 100 }}>
        <img src="/logo.jpg" alt="Angie's" onClick={() => { setActiveCategory(Object.keys(menu)[0] || ""); setSearch(""); setPage("menu"); }} style={{ height: 48, width: 48, borderRadius: "50%", objectFit: "cover", border: "2px solid #f5c842", flexShrink: 0, cursor: "pointer" }} />
              ...
              <div style={{ padding: "16px" }}>
                {loading ? (
                  ...
                  <div style={{ textAlign: "center", padding: 60, color: "#888" }}>
                    ...
                    <div style={{ fontSize: 13, marginTop: 8 }}>Admin থেকে menu add করো</div>
                  </div>
                ) : (
      ...
      {page === "checkout" && (
        <div className="fade-in" style={{ maxWidth: 520, margin: "0 auto", padding: "20px 16px 80px" }}>
          <button onClick={() => setPage("menu")} style={{ background: "none", border: "none", fontSize: 14, fontWeight: 700, cursor: "pointer", color: "#555", marginBottom: 20, fontFamily: "'Nunito',sans-serif" }}>← Back</button>
